...
where FGUID=@FGUID ";
        ...
        public IEnumerable<T_CONSUMABLESModel> GetConsumablesModes(IDictionary dic)
        {
            string sqlstr = "select * from T_CONSUMABLES where 1=1 ";
            DynamicParameters dp = new DynamicParameters();
            ...
            using (IDbConnection db = OpenConnection())
            {
                var result = db.Query<T_CONSUMABLESModel>(sqlstr, dp).AsList();
                return result;
            }
        }
    }
