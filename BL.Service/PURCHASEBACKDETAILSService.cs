   ...
   public class PURCHASEBACKDETAILSService : DBContext
    {
        ...
        public T_PURCHASEBACKDETAILSModel AddPURCHASEBACKDETAILS(T_PURCHASEBACKDETAILSModel model)
        {
            ...
            string sql = @"insert into  T_PURCHASEBACKDETAILS(FGUID, FCREATEID, FCREATETIME, FPARENTID, FGOODSID, FGOODSNAME, FUNIT, FQUANTITY, FMONEY, FPRICE, FSUPPLIERID, FMEMO
) values(@FGUID, @FCREATEID, @FCREATETIME, @FPARENTID, @FGOODSID, @FGOODSNAME, @FUNIT, @FQUANTITY, @FMONEY, @FPRICE, @FSUPPLIERID, @FMEMO
)";
            using (IDbConnection db = OpenConnection())
            {
